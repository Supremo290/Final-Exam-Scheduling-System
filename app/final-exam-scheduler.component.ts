...
editingExamKey: string | null = null; // Store UNIQUE identifier
...
get filteredSchedule(): ScheduledExam[] {
 ...
 get filteredCourseGrid(): CourseGrid[] {
  ...
  // Filter courses based on selectedCourseFilter
    ...
    const years: YearSlots[] = Array.from(existingYears).sort((a, b) => a - b).map(year => {
      ...
      this.timeSlots.forEach(slot => {
        ...
        slots[slot] = exams.filter(exam => {
          ...
          // Department filter (optional, if we want it in Course Grid)
...
loadActiveConfigurationFromCookies() {
  ...
  console.log('Cookie length:', activeTerm ? activeTerm.length : 0);  
  

  
  if (activeTerm && activeTerm.length === 8) {
    const term = (this.global.syear && this.global.syear.slice(-1)) || '1'; 
...
private loadScheduleForGroup(group: ExamGroup) {
  ...
  try {
    ...
    // Close loading FIRST, then wait before showing success
    Swal.close();
    ...
    html: `
      <div style="text-align: left; padding: 15px;">
        ...
        <div style="background: #fef3c7; padding: 12px; border-radius: 8px; border-left: 4px solid #f59e0b;">
          ...
          The old schedule will be permanently replaced once you generate and save a new one.
          </p>
        </div>
      </div>
    `,
        ...
        Swal.fire({
          ...
          html: `
            <div style="text-align: left; padding: 15px;">
              ...
              <div style="background: #fef3c7; padding: 12px; border-radius: 8px; border-left: 4px solid #f59e0b;">
                <p style="margin: 0; color: #92400e; font-size: 14px;">
                  The existing schedule may no longer match the new dates.
                </p>
              </div>
    ...
    html: `
      <div style="text-align: left; padding: 15px;">
        ...
        <div style="background: #fef3c7; padding: 12px; border-radius: 8px; border-left: 4px solid #f59e0b; margin-bottom: 15px;">
          <p style="margin: 0; color: #92400e; font-size: 14px;">
            A saved schedule exists with the old dates.
          </p>
        </div>
  ...
  Swal.fire({
    ...
    if (result.value) {
      ...
      if (group.termYear) {
        ...
        const scheduleKey = `examSchedule_${groupName}_${group.termYear}`;
        localStorage.removeItem(scheduleKey);
        console.log(`🗑️ Deleted schedule: ${scheduleKey}`);
        
  
        const oldFormatKey = `schedule_${groupName}_${group.termYear}`;
        localStorage.removeItem(oldFormatKey);
        console.log(`🗑️ Deleted old format: ${oldFormatKey}`);
        
        // Clear student mapping data
        this.sharedData.clearStudentMappingForGroup(groupName, group.termYear);
        console.log(`🗑️ Cleared student mapping for: ${groupName}`);
      }  
      // Also delete the "latest" schedule if it matches this group
      const latestSchedule = localStorage.getItem('examScheduleData');
      if (latestSchedule) {
        try {
          const parsed = JSON.parse(latestSchedule);
          const savedGroupName = parsed.examGroup && parsed.examGroup.name ? parsed.examGroup.name : '';
          
          if (savedGroupName === groupName) {
            localStorage.removeItem('examScheduleData');
            console.log(`Deleted latest schedule (matched group name)`);
          }
        } catch (e) {
          console.error('Error checking latest schedule:', e);
        }
      }
      ...
      // Clear cookies related to this group
      const lastSelectedGroup = this.cookieService.get('lastSelectedExamGroup');
      if (lastSelectedGroup) {
        try {
          const savedGroup = JSON.parse(lastSelectedGroup);
          if (savedGroup.name === groupName) {
            this.cookieService.delete('lastSelectedExamGroup');
            console.log(`Cleared cookie for: ${groupName}`);
          }
        } catch (e) {
      ...
      Swal.fire({
        ...
        html: `
          <div style="text-align: left; padding: 15px;">
            ...
            <p style="color: #16a34a; margin-top: 10px; font-size: 14px;">
              ✓ Exam group removed<br>
              ✓ Saved schedules deleted<br>
              ✓ Student mappings cleared
            </p>
          </div>
        `,
        type: 'success',
...
/**
 * Clean up all orphaned schedules (schedules without exam groups)
 */
cleanupOrphanedSchedules() {
  console.log('Cleaning up orphaned schedules...');
  
  const validGroupNames = new Set(this.savedExamGroups.map(g => g.name));
  const keysToRemove: string[] = [];
  
  // Scan localStorage for schedule keys
  for (let i = 0; i < localStorage.length; i++) {
    const key = localStorage.key(i);
    if (!key) continue;
    
    // Check for schedule keys
    if (key.startsWith('examSchedule_') || key.startsWith('schedule_')) {
      // Extract group name from key
      // Format: "examSchedule_GROUPNAME_TERMYEAR" or "schedule_GROUPNAME_TERMYEAR"
      const parts = key.split('_');
      if (parts.length >= 3) {
        const groupName = parts.slice(1, -1).join('_'); // Handle names with underscores
        
        if (!validGroupNames.has(groupName)) {
          keysToRemove.push(key);
          console.log(`  Found orphaned schedule: ${key}`);
        }
      }
    }
  }
  ...
  if (keysToRemove.length > 0) {
    console.log(`Cleaned up ${keysToRemove.length} orphaned schedule(s)`);
    this.showToast('Cleanup Complete', `Removed ${keysToRemove.length} orphaned schedule(s)`, 'success');
  } else {
...
getTermYearLabel(termYearCode: string): string {
  ...
  console.log('Converting term code:', termYearCode);
  ...
  if (/^\d{7}$/.test(termYearCode)) {
    ...
    const result = (termMap[termCode] || 'Unknown') + ' SY ' + year1 + '-' + year2; 
  ...
  if (/^\d{8}$/.test(termYearCode)) {
    ...
    const result = 'SY ' + year1 + '-' + year2; 
    console.log('Converted:', termYearCode, '→', result);
    return result;
  }
  
  console.warn('Unknown format:', termYearCode);
  return 'Unknown';
}
...
assignRoomByDepartment(exam: Exam, usedRoomsSet: Set<string>, roomsList: string[]): string | null {
  ...
  // "IF subject_id CONTAINS 'ARCH': building = 'C'"
  if (subjectId.includes('ARCH')) {
    console.log('ARCH subject detected: ' + exam.subjectId + ' - forcing Building C');
    ...
    console.warn(' Building C full, trying K fallback');
...
// filter available rooms based on department preferences
getAvailableRoomsForExam(exam: ScheduledExam, day: string, slot: string): string[] {
  console.log(`\nGetting available rooms for ${day} ${slot}`);
  
  if (!day || !slot) {
    console.log('No day or slot provided');
    return [];
  }

  const deptCode = exam.DEPT ? exam.DEPT.toUpperCase() : '';
  const course = exam.COURSE ? exam.COURSE.toUpperCase() : '';
  const subjectId = exam.SUBJECT_ID ? exam.SUBJECT_ID.toUpperCase() : '';

  // Determine preferred room prefixes
  let preferredPrefixes: string[] = [];
  
  if (subjectId.includes('ARCH')) {
    preferredPrefixes = ['C', 'K'];
    console.log('ARCH subject - Buildings C and K only');
  } else {
    if (deptCode === 'SABH') {
      preferredPrefixes = ['A'];
    ...
    console.log(`Preferred prefixes for ${deptCode}:`, preferredPrefixes);
  }

  // Get rooms already used in this slot (EXCLUDING current exam)
  ...
  console.log(`Used rooms in ${day} ${slot}:`, Array.from(usedRoomsForSlot));

  // Get all rooms
  const allRooms = this.rooms || [];
  console.log(`Total rooms available: ${allRooms.length}`);

  // Filter: Remove used rooms
  let availableRooms = allRooms.filter(room => !usedRoomsForSlot.has(room));
  console.log(`✓ Available after filtering used: ${availableRooms.length}`);
  ...
  if (preferredPrefixes.length > 0) {
    ...
    console.log(`Preferred rooms: ${preferredRooms.length}`);

    if (preferredRooms.length > 0) {
      // For ARCH, ONLY show C and K
      if (subjectId.includes('ARCH')) {
        console.log('Returning ARCH-only rooms');
        return preferredRooms;
      }
      
      // For others, show preferred first, then others
      const otherRooms = availableRooms.filter(room =>
        !preferredPrefixes.some(prefix => room.startsWith(prefix))
      );
      
      console.log('Returning preferred rooms first, then others');
      return [...preferredRooms, ...otherRooms];
    }
  }

  console.log('Returning all available rooms');
...
onEditSlotChange(): void {
  ...
  console.log('Slot changed to:', this.editedExam.SLOT);

  // Get conflict-free rooms for this day-slot
  ...
  console.log(`✓ ${this.availableRooms.length} available rooms:`, this.availableRooms);
  ...
  if (this.editedExam.ROOM && !this.availableRooms.includes(this.editedExam.ROOM)) {
    console.log('⚠️ Current room is not available, clearing selection');
...
// Show room recommendations in the UI
// Check if a room matches department preferences and show badge
...
// Check if there are recommended rooms available
...
// Save method with proper feedback
...
// Download instead of localStorage
...
private clearOldSchedulesAndRetry(saveData: any) {
  ...
  if (cleared > 0) {
    console.log(`Cleared ${cleared} old schedule(s)`);
...
// Detect only PROCTOR conflicts (for Proctor Assignment view)
  ...
  Object.entries(examsByDaySlot).forEach(([key, examsInSlot]) => {
    ...
    Object.entries(proctorCount).forEach(([proctor, exams]) => {
      if (exams.length > 1) {
        ...
        console.warn(`Proctor conflict: ${proctor} assigned to ${exams.length} exams at ${day} ${slot}`);
      }
    });
  });
...
// Detect only ROOM conflicts (for Generated Schedule view)
...
getTotalConflicts(): number {
  ...
  // Find available substitute proctors for an exam
  ...
  // Get proctor schedule (all exams a proctor is assigned to)
...
// Get match type badge with detailed matching
...
// Filter available proctors by department
...
// Filter available proctors by subject
...
// Refresh proctor view to ensure dropdowns update
refreshProctorView() {
...
startEdit(displayIndex: number) {
  ...
  console.log('Edit mode started for display index:', this.editingRow);
...
onEditDayChange() {
  ...
  console.log('Day changed to:', this.editedExam.DAY);

  //  Find conflict-free slots for this exam's course and year
  this.availableSlots = this.timeSlots.filter(slot => {
    // Check if this slot would create a conflict for the exam's course-year
    const wouldConflict = this.generatedSchedule.some(e => {
      // Skip the exam being edited
      if (e.CODE === this.editedExam.CODE && 
          e.SUBJECT_ID === this.editedExam.SUBJECT_ID &&
          e.DAY === this.editedExam.DAY) {
        return false;
      }

      // Check for course-year conflict at this day-slot
      return e.DAY === this.editedExam.DAY &&
             e.SLOT === slot &&
             e.COURSE === this.editedExam.COURSE &&
             e.YEAR_LEVEL === this.editedExam.YEAR_LEVEL;
    });

    return !wouldConflict;
  });

  console.log(`✓ ${this.availableSlots.length} conflict-free slots available:`, this.availableSlots);

  // Clear slot and room if current selections are invalid
  if (this.editedExam.SLOT && !this.availableSlots.includes(this.editedExam.SLOT)) {
    console.log('⚠️ Current slot is not available, clearing selection');
    this.editedExam.SLOT = '';
    this.editedExam.ROOM = '';
  }

  // Clear rooms list when day changes
  this.availableRooms = [];
...
saveEdit() {
  ...
  // Find ALL matching entries in generatedSchedule (handles multi-slot)
  const matchingExams = this.generatedSchedule.filter(e => 
  ...
  if (matchingExams.length === 0) {
    console.error(' No matching exams found');
    this.showToast('Error', 'Exam not found in schedule', 'destructive');
    return;
  }

  const original = matchingExams[0];
  ...
  //  Clear old room usage for ALL matching exams
  matchingExams.forEach(e => {
    const key = `${e.DAY}_${e.SLOT}`;
    if (this.usedRoomsPerSlot[key] && e.ROOM) {
      this.usedRoomsPerSlot[key].delete(e.ROOM);
      console.log(`  Cleared old room ${e.ROOM} from ${key}`);
    }
  });

  // Remove ALL old entries (don't leave any behind)
  ...
  console.log(`Removed ${matchingExams.length} old entry/entries`);

  // Add new entries based on whether it's multi-slot or not
  if (isMultiSlot && totalSlots > 1) {
    ...
    if (newSlotIndex === -1) {
      ...
      // Restore the schedule since we already removed entries
      this.generatedSchedule.push(...matchingExams);
      return;
    }
    ...
    for (let i = 0; i < totalSlots; i++) {
      ...
      this.generatedSchedule.push({
        ...
        SUBJECT_ID: this.editedExam.SUBJECT_ID,
        DESCRIPTIVE_TITLE: this.editedExam.DESCRIPTIVE_TITLE,
        COURSE: this.editedExam.COURSE,
        YEAR_LEVEL: this.editedExam.YEAR_LEVEL,
        INSTRUCTOR: this.editedExam.INSTRUCTOR,
        DEPT: this.editedExam.DEPT,
        DEPT_SUB: this.editedExam.DEPT_SUB,
        OE: this.editedExam.OE,
        DAY: this.editedExam.DAY,
        SLOT: slot,
        ROOM: this.editedExam.ROOM,
        PROCTOR: this.editedExam.PROCTOR || this.editedExam.INSTRUCTOR,
        HAS_CONFLICT: false,
        IS_MULTI_SLOT: true,
        SLOT_INDEX: i,
      ...
      console.log(` ✓ Added slot ${i + 1}/${totalSlots}: ${slot}`);
    }

    console.log(`Updated ${totalSlots} slot entries for multi-slot exam`);
  } else {
    // === SINGLE SLOT: Add one entry ===
  ...
  // Rebuild everything to ensure consistency
...
// Optimized auto-save that only saves essential data
autoSaveToLocalStorage() {
  try {
    ...
    const minimalData = {
      ...
      //  Only save generated schedule (NOT raw exams)
      generatedSchedule: this.generatedSchedule,
    ...
    if (sizeInKB > 4500) { // 4.5MB threshold
      console.warn('Data too large for auto-save');
      ...
      html: `
        <div style="text-align: left; padding: 15px;">
          ...
          <p style="margin-top: 15px; color: #d99594; font-size: 13px;">
             This will replace your current schedule (if any).
          </p>
        </div>
      `,
      type: 'question',
...
assignProctors() {
  ...
  // Assign main instructor to first exam in each group
  ...
  // Fill in substitutes for blank proctors
  this.generatedSchedule.forEach(exam => {
    if (!exam.PROCTOR) {
  ...
  // Recompute the available proctor list for dropdowns
  this.computeAvailableProctors();
}


// Assign proctor button handler
...
this.detectProctorConflicts();
this.generateCourseGridData();
this.synchronizeSameCodeExams();  // ✅ Generate grid data here
...
/**
 * Synchronizes all exams that share the same CODE to have the same day, time, and room
 * This ensures all sections of the same subject are scheduled together
 */
synchronizeSameCodeExams() {
  console.log('🔄 Synchronizing all exams with same codes...');
  
  if (!this.generatedSchedule || this.generatedSchedule.length === 0) {
    console.log('No schedule to synchronize');
    return;
  }
  
  // Group exams by CODE
  const examsByCode: { [code: string]: ScheduledExam[] } = {};
  
  this.generatedSchedule.forEach(exam => {
    const code = exam.CODE ? exam.CODE.toString().trim() : '';
    if (!code) return;
    
    if (!examsByCode[code]) {
      examsByCode[code] = [];
    }
    examsByCode[code].push(exam);
  });
  
  // Find codes that have multiple exams (sections)
  const codesWithMultipleSections = Object.entries(examsByCode)
    .filter(([code, exams]) => exams.length > 1);
  
  if (codesWithMultipleSections.length === 0) {
    console.log('✓ No codes with multiple sections found');
    return;
  }
  
  console.log(`Found ${codesWithMultipleSections.length} codes with multiple sections`);
  
  let totalSynced = 0;
  const syncDetails: { code: string; count: number; day: string; slot: string; room: string }[] = [];
  
  // Synchronize each group
  codesWithMultipleSections.forEach(([code, exams]) => {
    console.log(`\n📋 Synchronizing code ${code} (${exams.length} sections)`);
    
    // Use the first exam as reference
    const referenceExam = exams[0];
    const targetDay = referenceExam.DAY;
    const targetSlot = referenceExam.SLOT;
    const targetRoom = referenceExam.ROOM;
    
    console.log(`  Reference: Day ${targetDay}, Slot ${targetSlot}, Room ${targetRoom}`);
    
    let sectionsSynced = 0;
    
    // Update all other exams in this group
    exams.forEach((exam, index) => {
      if (index === 0) return; // Skip reference exam
      
      const oldDay = exam.DAY;
      const oldSlot = exam.SLOT;
      const oldRoom = exam.ROOM;
      
      // Check if already synchronized
      if (oldDay === targetDay && oldSlot === targetSlot && oldRoom === targetRoom) {
        console.log(`  ℹ️ ${exam.COURSE} already synchronized`);
        return;
      }
      
      // Clear old room usage
      if (oldRoom && oldRoom !== 'TBD' && oldRoom !== 'Please assign room') {
        const oldKey = `${oldDay}_${oldSlot}`;
        if (this.usedRoomsPerSlot[oldKey]) {
          this.usedRoomsPerSlot[oldKey].delete(oldRoom);
        }
      }
      ...
      exam.ROOM = targetRoom;
      
      // Mark new room as used (only if it's a valid room)
      if (targetRoom && targetRoom !== 'TBD' && targetRoom !== 'Please assign room') {
        const newKey = `${targetDay}_${targetSlot}`;
        if (!this.usedRoomsPerSlot[newKey]) {
          this.usedRoomsPerSlot[newKey] = new Set();
        }
        this.usedRoomsPerSlot[newKey].add(targetRoom);
      }
      
      sectionsSynced++;
      console.log(`  ✓ Updated ${exam.COURSE} from ${oldDay} ${oldSlot} ${oldRoom} → ${targetDay} ${targetSlot} ${targetRoom}`);
    });
    ...
    if (sectionsSynced > 0) {
      ...
      totalSynced += sectionsSynced;
    }
  });
  
  if (totalSynced > 0) {
    console.log(`\n✅ Total synchronized: ${totalSynced} exam(s) across ${syncDetails.length} code(s)`);
    ...
    this.showSyncNotification(syncDetails, totalSynced);
  } else {
    console.log('✓ All exams already synchronized');
    this.showToast('Already Synchronized', 'All exams with same codes are already scheduled together', 'info');
  }
}

/**
 * Show detailed notification about synchronized exams
 */
private showSyncNotification(syncDetails: { code: string; count: number; day: string; slot: string; room: string }[], totalSynced: number) {
  const detailsHtml = syncDetails.map(detail => 
    `<li><strong>Code ${detail.code}</strong>: ${detail.count} sections → ${detail.day} ${detail.slot} (Room ${detail.room})</li>`
  ).join('');
  
  Swal.fire({
    title: '✅ Exams Synchronized!',
    html: `
      <div style="text-align: left; padding: 15px;">
        <p style="margin-bottom: 15px;">
          <strong>Synchronized ${totalSynced} exam(s) across ${syncDetails.length} code(s)</strong>
        </p>
        <div style="max-height: 300px; overflow-y: auto; background: #f0fdf4; padding: 10px; border-radius: 8px;">
          <ul style="margin: 0; padding-left: 20px; font-size: 14px;">
            ${detailsHtml}
          </ul>
        </div>
        <p style="margin-top: 15px; color: #16a34a; font-size: 13px;">
          ✓ All sections with the same code are now scheduled at the same time and room
        </p>
      </div>
    `,
    type: 'success',
...
/**
 * Check if there are any codes with multiple sections
 */
hasCodesWithMultipleSections(): boolean {
  if (!this.generatedSchedule) return false;
  ...
  return Object.values(codeCounts).some(count => count > 1);
}
